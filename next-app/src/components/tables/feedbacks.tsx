  ...
  const columns = React.useMemo<ColumnDef<Feedback>[]>(
    () => [
      ...
      },
      {
        accessorKey: 'rating',
        header: 'Rating',
        cell: info => <Ratings rating={Number(info.getValue())} />,
        footer: props => props.column.id,
      },
      {
          ...
          {table.getHeaderGroups().map(headerGroup => (
            <tr key={headerGroup.id} className='border-b border-slate-300'>
              {headerGroup.headers.map(header => {
                return (
                  <th key={header.id} colSpan={header.colSpan} className='text-left bg-gray-50 rounded-t-md p-4 pb-2'>
                    <div
                      {...{
            ...
            return (
              <tr key={row.id} className='border-b'>
                {row.getVisibleCells().map(cell => {
                  return (
                    <td key={cell.id} style={{ width: cell.column.getSize() }} className='p-4'>
                      {flexRender(
